    ...
    if user_file and os.path.exists(user_file):
        ...
        try:
            ...
            if not df_hist.empty:
                ...
                last_data["PF"] = int(last_row.get("PF", 0)) if "PF" in last_row else 0

                last_data["ESIC"] = int(last_row.get("ESIC", 0)) if "ESIC" in last_row else 0

                last_data["Bonus"] = int(last_row.get("Bonus", 0)) if "Bonus" in last_row else 0

                last_data["Advance"] = int(last_row.get("Advance", 0)) if "Advance" in last_row else 0
...
if st.button("Calculate & Save Data", type="primary", use_container_width=True):
    ...
    if not emp_sidebar_name:
        ...
        try:
            ...
            # નવો ગણતરીનો ફેરફાર: બોનસ ઉમેરાય છે, બાકી બધું બાદ થાય છે
...
if emp_sidebar_name:
    ...
    if os.path.exists(user_file):
        ...
        try:
            ...
            st.info("💡 Tip: You can edit values directly, or select a row's checkbox on the far left and press 'Delete' to remove it.")

            # dataframe ની જગ્યાએ data_editor નો ઉપયોગ કરો
            ...
            with col_btn2:
                ...
                if st.button("💾 Save Changes to File", type="primary", use_container_width=True):
        ...
        # આ નવો સુધારો છે જેથી st.rerun() ને એરર ના સમજે 

        except pd.errors.EmptyDataError:

            st.warning("ફાઈલમાંથી બધો ડેટા ડીલીટ થઈ ગયો છે. નવો રેકોર્ડ ઉમેરો.")

        except Exception as e:

            st.error(f"ફાઈલ વાંચવામાં સમસ્યા છે: {e}")
